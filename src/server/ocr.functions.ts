...
function parseTextWithRegex(text: string): Partial<OcrResult> {
  ...
  const montant_ttc = amounts[0] ?? 0;
  const montant_ht = amounts[1] ?? montant_ttc / 1.2;
  ...
  return {
    ...
    montant_tva,
    montant_ttc,
    lignes: montant_ttc > 0
      ? [{ designation: "Prestation", quantite: 1, prix_unitaire: montant_ht, taux_tva: 20 }]
      : [],
  };
}
...
// ─── AI fallback (Claude Haiku — cheapest Anthropic model) ───────────────────
...
Voici le texte extrait (peut être incomplet) : 
<texte>${text.slice(0, 2000)}</texte>
${imageBase64 ? "Tu as aussi accès à l'image originale." : ""}
...
Extrais et retourne UNIQUEMENT ce JSON (sans markdown, sans explication) :
{
  ...
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      model: "claude-haiku-4-5-20251001", // cheapest, fast, precise
      max_tokens: 1024,
      messages,
    }),
  });
  ...
  if (!response.ok) {
    ...
    throw new Error(`Claude API ${response.status}: ${txt}`);
  }
...
export const runOcr = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Step 2: AI fallback if confidence is low or medium
    if (confidence === "low" || confidence === "medium") {
      const apiKey = process.env.ANTHROPIC_API_KEY;
      if (!apiKey) {
        // No API key — return regex result with low confidence
        ...
        try {
          ...
          // Merge: AI fills gaps in regex result
          finalResult = {
